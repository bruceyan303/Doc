...
//3）RTC定时唤醒(不受选型里clock的影响),唤醒IDEL
//===================================================================================//
//==============================================================================//
//头文件	运用头文件	通用寄存器定义文件
//==============================================================================//
...
//RTC定时唤醒时间T = (1/32.768KHz)*4Clock*2*256 = 62.5ms 
//------------------------------------------------------------------
void file_RTC_wakeIDEL_set()
{
	CONTW(0X80);			//设置TCC预分频1:2,bit7:RTCS使0x84;bit7=1:选择外部晶振时钟(LXT的4分频） 
	TCC=0;					//定时中断设置T = (1/32.768KHz)*4Clock*2*256 = 62.5ms 
	NOP();
	IDLE=1;					//1:系统执行 SLEEP 指令时进入空闲模式，系统时钟正常工作
	TCCCKS=1;				//TCC 时钟源选择,1:选择系统时钟
	TCCWE=1;				//1:TCC唤醒使能,可唤醒空闲模式,RTC 模式下可唤醒睡眠以及空闲模式
