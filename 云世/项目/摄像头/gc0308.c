...
static const struct regval_list gc0308_init_regs[] = {
    ...
    {0xfe, 0x00},
    {0x01, 0x6a}, // Hb horizontal blanking
    {0x02, 0x70}, // Vb vertial blanking
    {0x0f, 0x00},
...
static struct regval_list gc0308_mirror_h_regs[] = {
    {0xfe, 0x00},
    {0x14, 0x10},
    ENDMARKER,
};

static struct regval_list gc0308_mirror_v_regs[] = {
    {0xfe, 0x00},
    {0x14, 0x10},
    ENDMARKER,
};
...
static int s_ctrl_mirror_vflip(struct i2c_client *client, int value)
{
    ...
    gc0308_mirror_v_regs[1].value &= ~REG_VFLIP_IMG;
    gc0308_mirror_v_regs[1].value |= val;
    ret = gc0308_write_array(client, gc0308_mirror_v_regs);
    if (ret < 0)
    {
...
static int s_ctrl_mirror_hflip(struct i2c_client *client, int value)
{
    ...
    gc0308_mirror_h_regs[1].value &= ~REG_HFLIP_IMG;
    gc0308_mirror_h_regs[1].value |= val;
    ret = gc0308_write_array(client, gc0308_mirror_h_regs);
    if (ret < 0)
    {
        dev_err(&client->dev, "Failed to write mirror regs\n");
        return -1;
...
static int gc0308_video_probe(struct i2c_client *client)
{
    ...
    ret = gc0308_read_reg(client, 0x00);
    printk(KERN_ERR "can't two times?==ret=%x\n", ret);
                             ...
                             struct v4l2_dbg_register *reg)
{
    ...
    printk(KERN_ERR "[kernel]:GC0308_read==\n");
    reg->size = 1;
    if (reg->reg > 0xff)
        return -EINVAL;

    printk(KERN_ERR "==reg=%x\n", (u8)(reg->reg));
    ret = gc0308_read_reg(client, (u8)(reg->reg));
    if (ret < 0)
        return ret;
    reg->val = (__u64)ret;
    printk(KERN_ERR "gc0308_g_register===val=0x%x\n", reg->val);
    return 0;
}
                             ...
                             const struct v4l2_dbg_register *reg)
{
    ...
    int ret;
    printk(KERN_ERR "[kernel]:GC0308_write==\n");

    if (reg->reg > 0xff ||
        reg->val > 0xff)
        return -EINVAL;

    ret = gc0308_write_reg(client, (u8)(reg->reg), (u8)(reg->val));
    return ret;
    printk(KERN_ERR "gc0308_s_register===val=0x%x\n", ret);
}
#endif
