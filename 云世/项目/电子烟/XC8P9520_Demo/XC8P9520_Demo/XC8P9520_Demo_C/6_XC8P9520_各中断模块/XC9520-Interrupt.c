...
//---------------------------------
//file_exInt_set:
	IOCP_W(P6CR,0x01);		//P60输入口
	IOCP_W(WDTCR,0x40);		//Bit<6>:EIS-P60 外部中断使能位,设置P60为中断脚
	IOCP_W(IMR,0x05);		//TCC+P60外部中断/* */ 			
//---------------------------------
//P6端口变化中断 
//---------------------------------
//file_p6Int_set:  
/* 	ICIECR = 0x08;			//P63端口独立唤醒
	IOCP_W(IMR,0x03);		//TCC+P6端口变化中断*/ 	
//---------------------------------
//PWM周期中断中断 
//---------------------------------
//file_pwmInt_set:  
/*	IOCP_W(IMR,0x09);		//TCC+PWM周期中断 	
	PWMCON = 0x88;			//pwm=1/8*2(分频)*4clock*100=100us
	PRD = 100;				//PWM周期寄存器*/ 
}
...
//=========Tcc中断程序===============//
	...
	if(EXIF == 1)					//判断EXIF是否为1,外部中断
	{	 
		ISR    = 0xfb;				//清EXIF中断标志位
		PORT6_4 = !PORT6_4;			//P64口翻转
	}
	if(ICIF == 1)					//判断ICIF是否为1,端口变化中断
	{	 
		ISR    = 0xfd;				//清ICIF中断标志位
		PORT6_4 = !PORT6_4;			//P64口翻转
	}
